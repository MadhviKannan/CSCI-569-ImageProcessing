...
int main()
{
    ...
    int BytesPerPixel=3;
    int i,j;
    if (!(file1=fopen("letterE.raw","rb"))) {
		cout << "Cannot open file: " << "letterE.raw" <<endl;
		exit(1);
	}
...
//---------String table of conditional masks used
...
//------start thinning operation------//
string str1="";
string str2="";
int count1=0;
int c1=0;
for(int run=0;run<90;run++) //90 iterations
{
for(int i=0;i<477;i++)
{
for(int j=0;j<477;j++)
{
        for(int m=0;m<3;m++)
        {
            for(int n=0;n<3;n++)
            {
                str1=str1+(char)(temp_mat[m+i][n+j]+48); //string of 9 bits generation for the intermediate matrix calculation
            }
        }
        int mid_val=(int)temp_mat[i+1][j+1];
        if(mid_val!=1) //intermediate matrix generation checking condition
        {
...
cout<<final1<<endl;
cout<<h<<endl;
if (!(file=fopen("letterE_inter.raw","wb"))) {
		cout << "Cannot open file: " << "letterE_inter.raw" << endl;
		exit(1);
	}

	fwrite(square_inter, sizeof(unsigned char),   480*480, file);
	printf("File written\n");
	fclose(file);

if (!(file=fopen("letterE_thin.raw","wb"))) {
		cout << "Cannot open file: " << "letterE_thin.raw" << endl;
		exit(1);
	}
