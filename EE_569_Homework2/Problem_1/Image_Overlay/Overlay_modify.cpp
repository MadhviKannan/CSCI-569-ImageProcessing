...
int main()
{
    ...
    if (!(file1=fopen("trojans.raw","rb"))) {
		cout << "Cannot open file: " << "trojans.raw" <<endl;
		exit(1);
	}
	fread(trojans,sizeof(unsigned char), 350*197*BytesPerPixel, file1);
    fclose(file1);
   ...
   double h[3][3]={0.1896,1.4668,-234.8454,-0.5248,0.1734,340.6749,0.0003,-0.0031,1.0};
   double x[972]={0.0};
   double y[648]={0.0};
   double x_new,y_new;
        ...
        for(int j=0;j<972;j++)
        {
            for(int k=0;k<3;k++)
            {
             ...
             if((x_new>=(((137.0/102.0)*(y_new-109.5))+393.5))&&y_new<=(((-6.0/186.0)*(x_new-530.5))+211.5)&&x_new<=(((44.0/132.0)*(y_new-205.5))+716.5)&&y_new>=(((-36.0/279.0)*(x_new-672.5))+73.5))
             {
                    ...
                    if((int)trojans[x_field][y_field][0]<253&&(int)trojans[x_field][y_field][1]<253&&(int)trojans[x_field][y_field][2]<253)
{
    ...
    for(int i=0;i<648;i++)
    {
        for (int j=0;j<972;j++)
        {
            for(int k=0;k<3;k++)
            {
                if((int)new_field[i][j][k]>245)
                {
                    field_inter[i][j][k]=field[i][j][k];
                }
                else
                {
                    field_inter[i][j][k]=new_field[i][j][k];
                }
            }
        }
