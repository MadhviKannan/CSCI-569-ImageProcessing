...
#include<math.h>
#include<tgmath.h>
unsigned char forest[600][960][3];
unsigned char forest_transform[600][960][3];
using namespace std;
int main()
{
	...
	if (!(file1=fopen("forest_1.raw","rb"))) {
		cout << "Cannot open file: " << "forest_1.raw" <<endl;
		exit(1);
	}
	fread(forest,sizeof(unsigned char), 600*960*BytesPerPixel, file1);
    fclose(file1);
    ...
    for(int j=0;j<600;j++)
        ...
        for(int y=0;y<256;y++)
        {
            ...
            prob_arr_red[y]=(double)count_array_red[y]/(600*960);
            prob_arr_green[y]=(double)count_array_green[y]/(600*960);
            prob_arr_blue[y]=(double)count_array_blue[y]/(600*960);
...
if (!(file2=fopen("forest_enhanced.raw","wb"))) {
	...
	fwrite(forest_transform, sizeof(unsigned char),   600*960*BytesPerPixel, file2);
	printf("File written\n");
	fclose(file2);
    return 0;
}
