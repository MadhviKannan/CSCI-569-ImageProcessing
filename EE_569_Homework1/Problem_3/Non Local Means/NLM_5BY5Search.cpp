...
# Name  : Madhvi Kannan
# USC ID: 2159965290
# email : mkannan@usc.edu#include <iostream>
#include<stdio.h>
#include<stdlib.h>
#include<fstream>
#include<math.h>
...
for(int row=2;row<514;row++)
    {
    for(int col=2;col<514;col++)
        {
            for(int x=0;x<3;x++)
        {
    for(int temp_row=row-2;temp_row<row+1;temp_row++) //big sliding window
    {
    for(int temp_col=col-2;temp_col<col+1;temp_col++)
    {
        for(int small_row=0;small_row<3;small_row++) //small fixed window
        {
            for(int small_col=0;small_col<3;small_col++)
            {
           sum_val=sum_val+(int)pepper_extend[temp_row+small_row][temp_col+small_col][x];
        }

        }
         image[temp1]=(int)pepper_extend[temp_row+1][temp_col+1][x];
           temp1++;
        vec[temp]=(double)sum_val/9.0;
        temp++;
        sum_val=0;
        }
        }
        ...
        for(int h=0;h<9;h++)
        {
              norm=norm+exp(-1*(fabs(pow((vec[4]-vec[h]),2.0))/(45.0*45.0)));
        }
        for(int d=0;d<9;d++)
        {
            weight[d]= weight[d]=exp(-1*(fabs(pow((vec[4]-vec[d]),2.0))/(80.0*80.0)));
            final_val= final_val+(weight[d]*image[d]);

        }

        pepper_denoised[row-2][col-2][x]=(int)(final_val/norm);
        norm=0;
        final_val=0.0;
        }
}
