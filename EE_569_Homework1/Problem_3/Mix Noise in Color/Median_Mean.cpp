...
for(int row=0;row<512;row++)
    ...
    for(int col=0;col<512;col++)
        ...
        pepper_denoised_median[row][col][x]=temp[4];
         ...
         if (!(file3=fopen("pepper_denoised_median.raw","wb"))) {
	...
	printf("File written\n");
	fclose(file3);
    if (!(file1=fopen("pepper_denoised_median.raw","rb"))) {
		cout << "Cannot open file: " << "pepper_denoised_median.raw" <<endl;
		exit(1);
	}
	fread(pepper_denoised_median,sizeof(unsigned char), 512*512*BytesPerPixel, file1);
    fclose(file1);
    int temp_row=0;
    int temp_col=0;
    int sum_val=0;
...
for(int row=0;row<514;row++)
    ...
    for(int temp_col=0;temp_col<3;temp_col++)
        {
            sum_val=sum_val+((int)pepper_denoised_median[row+temp_row][col+temp_col][x]);
        }
        }
    ...
    if (!(file3=fopen("pepper_denoised_medianmean.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_denoised_medianmean.raw" << endl;
		exit(1);
	}
