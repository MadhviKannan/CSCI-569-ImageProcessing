...
unsigned char pepper_extend[514][514][3];
...
for(int row=0;row<514;row++)
    ...
    for(int temp_col=0;temp_col<3;temp_col++)
        ...
        pepper_denoised[row][col][x]=(int)((1.0/9.0)*sum_val);
               sum_val=0;
    }
        }
    }
...
for(int row=0;row<512;row++)
    ...
    for(int col=0;col<512;col++)
        ...
        pepper_denoised_median[row][col][x]=temp[4];
        }
    }
        }
        //Gaussian filter
...
for(int row=0;row<514;row++)
    ...
    if (!(file2=fopen("pepper_extend.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_extend.raw" << endl;
		exit(1);
	}
	fwrite(pepper_extend, sizeof(unsigned char),   514*514*BytesPerPixel, file2);
	printf("File written\n");
	fclose(file2);

    if (!(file3=fopen("pepper_denoised.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_denoised.raw" << endl;
		exit(1);
	}
	fwrite(pepper_denoised, sizeof(unsigned char),   512*512*BytesPerPixel, file3);
	printf("File written\n");
	fclose(file3);
	if (!(file4=fopen("pepper_denoised_median.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_denoised_median.raw" << endl;
		exit(1);
	}
	fwrite(pepper_denoised_median, sizeof(unsigned char),   512*512*BytesPerPixel, file4);
	printf("File written\n");
	fclose(file4);
	if (!(file5=fopen("pepper_denoised_gauss.raw","wb"))) {
	...
	printf("File written\n");
	fclose(file5);
	if (!(file6=fopen("pepper_denoised.raw","rb"))) {
		cout << "Cannot open file: " << "pepper_denoised.raw" <<endl;
		exit(1);
	}
	fread(pepper_denoised,sizeof(unsigned char), 512*512*BytesPerPixel, file6);
    fclose(file6);
	double val_mse=0.0;
	double psnr_red=0.0;
