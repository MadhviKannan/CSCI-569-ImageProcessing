...
for(int row=0;row<514;row++)
	...
	printf("File written\n");
	fclose(file3);
    if (!(file1=fopen("pepper_denoised.raw","rb"))) {
		cout << "Cannot open file: " << "pepper_denoised.raw" <<endl;
		exit(1);
	}
	fread(pepper_denoised,sizeof(unsigned char), 512*512*BytesPerPixel, file1);
    fclose(file1);
    //Median filter
     int temp[9]={0};
     int run;
...
for(int row=0;row<512;row++)
    ...
    for(int col=0;col<512;col++)
        ...
        for(int temp_row=0;temp_row<3;temp_row++)
            ...
            for(int temp_col=0;temp_col<3;temp_col++)
            {
            temp[run]=pepper_denoised[row+temp_row][col+temp_col][x];
            run=run+1;
        }
        }
        for(int i=0;i<9;i++)
        ...
        if (!(file5=fopen("pepper_cascade_meanmedian.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_cascade_meanmedian.raw" << endl;
		exit(1);
	}
