...
for(int row=0;row<514;row++)
    ...
    for(int temp_col=0;temp_col<3;temp_col++)
        {
            sum_val=sum_val+((int)pepper_denoised_median[row+temp_row][col+temp_col][x]*gauss[temp_row][temp_col]);
        }
        }
    ...
    if (!(file3=fopen("pepper_denoised_mediangauss.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_denoised_mediangauss.raw" << endl;
		exit(1);
	}
